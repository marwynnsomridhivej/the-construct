...
from ui import (ConfirmationModal, SettingsBindTextChannelModal,
                SettingsMapPoolCreateModal, SettingsMapPoolEditModal,
...
class SettingsSelectButtons(SettingsBaseButtons):
    ...
    @discord.ui.button(label=SettingsChoice.GENERAL.title(), style=discord.ButtonStyle.blurple)
    async def _select_general(self, interaction: discord.Interaction, button: discord.ui.Button):
    ...
    @discord.ui.button(label=SettingsChoice.MAP_POOL.title(), style=discord.ButtonStyle.blurple)
    async def _select_map_pool(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Mark interaction as deferred so buttons immediately become available
        await interaction.response.defer()
...
class SettingsGeneralButtons(SettingsBaseButtons):
    ...
    def init_buttons(self) -> None:
        ...
        back_button = discord.ui.Button(
            label="Back", style=discord.ButtonStyle.grey)
    ...
    async def set_admins_button_callback(self, interaction: discord.Interaction):
        ...
        previous_admins = await self.parent_view.bot.settings_manager.get_admins(interaction.guild_id)
    ...
    async def bind_text_channel_button_callback(self, interaction: discord.Interaction):
        ...
        previous_channel_id = await self.parent_view.bot.settings_manager.get_bound_text_channel_id(interaction.guild_id)
...
class SettingsMapPoolButtons(SettingsBaseButtons):
    ...
    async def create_button_callback(self, interaction: discord.Interaction):
        ...
        if not await self.parent_view.bot.settings_manager.can_create_map_pool(interaction.guild_id):
            return await interaction.response.send_message(Canned.ERR_SETTINGS_MAP_POOL_CAP, ephemeral=True)
        ...
        try:
            ...
            return await interaction.followup.send(Canned.ERR_SETTINGS_MAP_POOL_NAME_TAKEN, ephemeral=True)
        else:
    ...
    async def edit_button_callback(self, interaction: discord.Interaction):
        ...
        pool = await self.parent_view.bot.settings_manager.get_map_pool(interaction.guild_id, name)
        ...
        if "maps" in changes:
            ...
            changes_text += f"\n### Maps\n" + "\n".join(sorted([
                f"[{"+" if r6map in edit_modal.maps else "-"}] {r6map.replace("_", " ").title()}" for r6map in diff
            ]))
